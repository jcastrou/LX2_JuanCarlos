...
do
	...
	cat $p | awk -F "\",\"" '{print $2}' | sed '1,$ s/"//g' | sed '2 s/Monto/#Monto/g'|head  > $GRAF_DATA/grafico-$m.dat
	let m=m+1
...
then
	...
	fi 2>/home/cujc/Administrador_de_Servidores/LX2_JuanCarlos/error/ErrorIf.log
...
do
	sed '4,$d' $k >> $FULL_DATA/agua.dat
 	echo "Procesando archivo $k,sacando datos de los 6 mese del agua"
        let m=m+1
        done 2>/home/cujc/Administrador_de_Servidores/LX2_JuanCarlos/error/ErrorConElFULL.log
